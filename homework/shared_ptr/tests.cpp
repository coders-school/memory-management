...
TEST_F(WeakPointerTest, shouldHaveNoReferencesAfterLockingWeakPointerWithSharedRef0)
{
    SharedPointer<int> s_ptrNew(new int(testValue));
    WeakPointer<int> w_ptr(s_ptrNew);

    s_ptrNew.reset();
    auto s2_ptr = w_ptr.lock();
    ASSERT_TRUE(w_ptr.expired());

    WeakPointer<int> w2_ptr(w_ptr);
    auto s3_ptr = w2_ptr.lock();
    ASSERT_TRUE(w2_ptr.expired());

    WeakPointer<int> w3_ptr(std::move(w_ptr));
    auto s4_ptr = w3_ptr.lock();
