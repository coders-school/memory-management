...
class BadWeakPointer : public std::runtime_error {
public:
    BadWeakPointer(std::string msg) : std::runtime_error{msg} {}
};
...
template <typename T>
SharedPointer<T>::SharedPointer(std::nullptr_t) : refCounter_(new ControlBlock<T>{}) {}

template <typename T>
SharedPointer<T>::SharedPointer(T* ptr) : ptr_(ptr), refCounter_(new ControlBlock<T>{ptr}) {
    if (ptr == nullptr) {
...
SharedPointer<T>::SharedPointer(WeakPointer<T>& weakPtr) : ptr_(weakPtr.ptr_), refCounter_(weakPtr.refCounter_) {
    if (weakPtr.expired()) {
        throw BadWeakPointer("Weak pointer expired !");
    }
